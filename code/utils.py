...
def heatmap_loss(combined_hm_preds, heatmaps, loss_fn= None, egl=False):
    '''
    ...
    for i in range(nstack):
        pred_heatmap = combined_hm_preds[:, i]
        gt_heatmap = heatmaps.to(combined_hm_preds[:, i].device)
        narrowed_pred = pred_heatmap.narrow(1, 0, gt_heatmap.size(1)) # for LSP with 14 ground truth predictions
        combined_loss.append(calc_loss(narrowed_pred, gt_heatmap))
...
def calculate_fourier_response(model):
    responses = []

    # Go through the model's layers
    for name, layer in model.named_modules():
        if isinstance(layer, torch.nn.Conv2d):
            # Get the filters from convolutional layers
            filters = layer.weight.data


            # Perform 2D Fourier transform on the filters (dimensions -2 and -1 correspond to kernel_height, kernel_width)
            fourier_response = torch.fft.fft2(filters, dim=(-2, -1)) # returns a complex number

            magnitude = torch.abs(fourier_response)
