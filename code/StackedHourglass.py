...
class PoseNet(nn.Module):
    def __init__(self, arch):#, nstack, inp_dim, oup_dim, bn=False, increase=0, **kwargs):
        '''
        ...
        self.outs = nn.ModuleList(
            [Conv(inp_dim=inp_dim, out_dim=self.oup_dim, kernel_size=1, relu=False, bn=False).cuda(cuda_devices[i])
             for i in range(self.nstack)])

        self.merge_features = nn.ModuleList([Merge(inp_dim, inp_dim).cuda(cuda_devices[i]) for i in range(self.nstack-1)])
        self.merge_preds = nn.ModuleList([Merge(self.oup_dim, inp_dim).cuda(cuda_devices[i]) for i in range(self.nstack-1)])
