...
def _uniquecounts(conf, dataset, selection, method, dataset_name):
    """
    ...
    with open(os.path.join(conf.model['save_path'], f'model_checkpoints/sampling_proportion{dataset_name}.txt'),
              "x") as file:
        file.write('{} Sampling\n'.format(method))
        [file.write("{}: {}\n".format(key, proportion[key])) for key in proportion.keys()]


def base(conf, dataset, train, dataset_size):
    """
    ...
    if dataset['load'] == 'mpii':
        selection = np.random.choice(unlabelled_idx, size=num_images, replace=False).astype(np.int32)

    elif dataset['load'] == 'lsp':
        # First sample from lsp dataset only
        lsp_images = min(dataset_size['lsp']['train'], num_images)

        selection_lsp = np.random.choice(np.arange(dataset_size['lsp']['train']), size=lsp_images, replace=False)

        selection = np.concatenate([selection_lsp], axis=0).astype(np.int32)

    else:
        # Merged dataset
        selection = np.random.choice(unlabelled_idx, size=num_images, replace=False).astype(np.int32)

    _uniquecounts(conf, dataset=train, selection=selection, method='base', dataset_name=dataset['load'])

    np.save(file=os.path.join(conf.model['save_path'], 'model_checkpoints/annotation.npy'), arr=selection)
...
def load_lsp(lsp_conf, model_conf):  # shuffle=False, train_ratio=0.7, conf=None):
    ...
    if model_conf['load']:
        train_test_split = np.load(os.path.join(model_conf['base_load_path'], 'model_checkpoints/lsp_split.npy'))
    else:
        train_test_split = np.concatenate(
            [np.zeros((int(annotation_lsp.shape[2] * lsp_conf['train_ratio']),), dtype=np.int8),
             np.ones((annotation_lsp.shape[2] - int(annotation_lsp.shape[2] * lsp_conf['train_ratio']),),
                     dtype=np.int8)],
            axis=0)
...
class HumanPoseDataLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_dict, conf, use_incr):
        """
        :param dataset_dict:
        :param conf:
        """
        ...
        self.dataset = conf.dataset_incr if use_incr else conf.dataset
        self.num_hm = 16 if self.dataset['load'] == 'mpii' else 14

        # Create dataset by converting into numpy compatible types
        if self.dataset['load'] in ['mpii', 'merged']:
        ...
        if self.dataset['load'] in ['lsp', 'merged']:
        ...
        elif self.dataset['load'] == 'lsp':
            self.train = self.merge_dataset(datasets=[self.lsp_train],
                                            indices=[np.arange(self.lsp_train['name'].shape[0])])
            self.validate = self.merge_dataset(datasets=[self.lsp_validate],
        ...
        if self.dataset['load'] in ['lsp', 'merged']:
            del self.lsp_train, self.lsp_validate, self.lsp_dataset, self.lsp,

        indices = base(conf=conf, dataset=self.dataset, train=self.train, dataset_size=self.dataset_size)
    ...
    def __getitem__(self, i):
        ...
        if self.train_flag:
            ...
            if torch.rand(1) < self.flip_prob:
                ...
                gt[:, :, :2] = np.stack(augmented['keypoints'], axis=0).reshape(-1,
                                                                                self.num_hm,
                                                                                2)
            ...
            # Else augmentation does not need to keep all joints
                ...
                augmented = self.shift_scale_rotate(image=image, keypoints=gt.reshape(-1, 3)[:, :2])
                image = augmented['image']
                gt[:, :, :2] = np.stack(augmented['keypoints'], axis=0).reshape(-1, self.num_hm, 2)
    ...
    def create_mpii(self):  # (self, train_ratio=0.7, max_persons=0, shuffle=False):
        ...
        dataset = {'img': [],
                   'name': [],
                   'gt': -np.ones(shape=(len_dataset, max_persons, self.num_hm, 3)),
                   'dataset': [],
        ...
        if self.conf.model['load']:
            dataset['split'] = np.load(os.path.join(self.conf.model['base_load_path'], 'model_checkpoints/mpii_split.npy'))

        else:
            # Create our own train/validation split for multi person dataset
            # 0: Train; 1: Validate

            if self.dataset['mpii_params']['newell_validation']:
                root = Path(os.getcwd()).parent
                ...
                train_ratio = self.dataset['mpii_params']['train_ratio']
                dataset['split'] = np.concatenate([np.zeros(int(len_dataset * train_ratio), ),
                                                   np.ones((len_dataset - int(len_dataset * train_ratio)), )],
                                                  axis=0)
                if self.dataset['mpii_params']['shuffle']: np.random.shuffle(dataset['split'])

        np.save(file=os.path.join(self.conf.model['save_path'], 'model_checkpoints/mpii_split.npy'),
                arr=dataset['split'])
    ...
    def create_lsp(self):
        ...
        dataset = {'img': [],
                   'name': [],
                   'gt': -np.ones(
                       shape=(len(lsp['img_name']), max_persons, self.num_hm, 3)),
                   'dataset': [],
                   ...
                   'bbox_coords': -np.ones(
                       shape=(len(lsp['img_name']), max_persons, 4))}  # max_persons is always 1 for lsp*
    ...
    def mpii_single_person_extractor(self, mpii_dataset):
        ...
        dataset = {'img': [],
                   'name': [],
                   'gt': np.empty(shape=(0, max_persons, self.num_hm, 3)),
                   'dataset': [],
        ...
        for i in range(len(mpii_dataset['name'])):
            for p in range(int(mpii_dataset['num_persons'][i][0])):
                ...
                dataset['gt'] = np.concatenate([dataset['gt'],
                                                gt_.reshape(1, max_persons, self.num_hm,
                                                            3)],
                                               axis=0)
